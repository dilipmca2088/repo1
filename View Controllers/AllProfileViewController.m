...
@interface AllProfileViewController ()
{
        int p1;
        int i;
...
- (void)viewDidLoad
{
    ...
    if ([_frmEditStr isEqualToString:@"T"])
    {
    ...
    [actHolNameTF setEnabled:NO];
    [actNumberTF setEnabled:NO];
    [ifscCodeTF setEnabled:NO];
    [bankNameTF setEnabled:NO];
    [branchNameTF setEnabled:NO];
    [actTypeTF setEnabled:NO];
    [panNoTF setEnabled:NO];

    [vendorNameTF setEnabled:NO];
    [vendoremailTF setEnabled:NO];
    [vendorstateTF setEnabled:NO];
    [vendorcityTF setEnabled:NO];
    [vendorcountryTF setEnabled:NO];
    [vendorAdd setEnabled:NO];
    
    [cityTF setEnabled:NO];
    [centreNameTF setEnabled:NO];
    [addressTF setEnabled:NO];
    [inchargeTF setEnabled:NO];
    [inchargeContTF setEnabled:NO];
    [inchargeEmailTF setEnabled:NO];
    [itPersonNameTF setEnabled:NO];
    [cityTF setEnabled:NO];
    [itPersonContactTF setEnabled:NO];
    [itPersonEmailTF setEnabled:NO];
    
    [systemAvailTF setEnabled:NO];
    
    [processorTF setEnabled:NO];
    
    [ramTF setEnabled:NO];
    
    [hdTF setEnabled:NO];
    [maleTF setEnabled:NO];
    [feMaleTF setEnabled:NO];
    [printerTF setEnabled:NO];
    [waitingTF setEnabled:NO];
    [parkingTF setEnabled:NO];
    [fstInternetTF setEnabled:NO];
    [fstIntSpeedTF setEnabled:NO];
    [secInternetTF setEnabled:NO];
    [secIntSpeedTF setEnabled:NO];
    [cctvTF setEnabled:NO];
    [acTF setEnabled:NO];
    
    [upspowerunitTF setEnabled:NO];
    [upsbackupTimeTF setEnabled:NO];
    [genpowerunitTF setEnabled:NO];
    [genFuelChargeTF setEnabled:NO];
    [noSysTF1 setEnabled:NO];
    [processorTF1 setEnabled:NO];
    [ramTF1 setEnabled:NO];
    [hd1 setEnabled:NO];
    
    [noSysTF2 setEnabled:NO];
    [processorTF2 setEnabled:NO];
    [ramTF2 setEnabled:NO];
    [hd2 setEnabled:NO];
    
    [noSysTF3 setEnabled:NO];
    [processorTF3 setEnabled:NO];
    [ramTF3 setEnabled:NO];
    [hd3 setEnabled:NO];
...
-(void)getCenterRequestWS
{
    ...
    [manager POST:[NSString stringWithFormat:@"%svendor_detailed/",URLBase] parameters:dict
         ...
         if ([responseCode isEqualToString:@"success"])
         {
            NSArray *examArray =[responseObject valueForKey:@"data"];
             ...
             else
             {
                 ...
                 vendorNameTF.text=[[[responseObject valueForKey:@"data"] valueForKey:@"vend_name"] objectAtIndex:0];
                 
                 vendoremailTF.text=[[[responseObject valueForKey:@"data"] valueForKey:@"vend_email"] objectAtIndex:0];
                 
                 vendorstateTF.text=[[[responseObject valueForKey:@"data"] valueForKey:@"vend_state"] objectAtIndex:0];
                 
                 vendorcityTF.text=[[[responseObject valueForKey:@"data"] valueForKey:@"vend_city"] objectAtIndex:0];
                 
                 vendorcountryTF.text=[[[responseObject valueForKey:@"data"] valueForKey:@"vend_coun"] objectAtIndex:0];
                 
                 vendorAdd.text=[[[responseObject valueForKey:@"data"] valueForKey:@"vend_post_add"] objectAtIndex:0];
...
- (void)textFieldDidBeginEditing:(UITextField *)textField
{
    ...
    if (textField==cityTF||textField==centreNameTF||textField==addressTF||textField==inchargeTF||textField==inchargeContTF||textField==inchargeEmailTF||textField==itPersonNameTF||textField==itPersonContactTF||textField==systemAvailTF||textField==processorTF||textField==hdTF||textField==hdTF)
    {
        ...
        _submitBtn.frame =CGRectMake(40, 515, 250, 30);
...
- (IBAction)radioBtnAction:(id)sender
{
    ...
    if (radioBtn==4)
    {
       ...
       // termsView.frame=CGRectMake(9,1640,307,53);
        ...
        _bankscrollView.frame=CGRectMake(-8,2050,340,391);
    ...
    if (radioBtn==5)
    {
       // termsView.frame=CGRectMake(42,1490,240,70);
        self.scrollView.contentSize =CGSizeMake(320,2300);
        _bankscrollView.frame=CGRectMake(-8,1908,340,391);
