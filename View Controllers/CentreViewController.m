...
- (void)textFieldDidBeginEditing:(UITextField *)textField
{
    ...
    if (textField==noSysTF1||textField==processorTF1||textField==ramTF1||textField==hd1)
    {
...
- (IBAction)registerBtnTpd:(id)sender
{
    ...
    if ([checkUpdateInsert isEqualToString:@"T"])
    {
        ...
        else if (checkSecImgCounter<4)
        {
            message = @"Upload Minimum 4 files in Secondary Images";
...
-(void)loadRegisterUserWS
{
    ...
    [manager POST:@"http://culinkservices.com/api/example/centerdetailed/" parameters:dict
constructingBodyWithBlock:^(id<AFMultipartFormData> formData)
     {
...
- (IBAction)radioBtnAction:(id)sender
{
   ...
   NSLog(@"%d", (int)[button tag]);
    ...
    if (radioBtn==5)
    {
        termsView.frame=CGRectMake(9,1490,240,70);
        self.scrollView.contentSize =CGSizeMake(320,1600);
        ...
        if ([dict objectForKey:UIImagePickerControllerMediaType] == ALAssetTypePhoto)
        {
            if ([dict objectForKey:UIImagePickerControllerOriginalImage]){
                ...
                if (camMatch==2)
                {
                    ...
                    checkSecImgCounter=(int)[images count];
